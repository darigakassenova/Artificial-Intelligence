    ...
    def __init__(self, root="./data", split="train", transform=None):
        ...
        if transform is None:
            transform = transforms.Compose([
                transforms.Resize(224), #BECAUSE MOBILENET expects this dimention
                transforms.ToTensor(),
                transforms.Normalize(  # statistics of thr data the model was originally trained with
...
def run_epoch(model, dataloader, text_emb, class_words, label_to_word, optimizer, temperature, device, mode='train'):
    ...
    with torch.no_grad() if mode == 'eval' else torch.enable_grad():
        desc = "[TRAIN]" if mode == "train" else "[EVAL]"
        for images, labels in tqdm(dataloader, desc=desc):
        # for images, labels in tqdm(dataloader, desc=f"[{model.upper()}]" if mode =="train" else None):
...
def train_with_early_stopping(model, dataloaders, text_emb, class_words, label_to_word, config, device):
    ...
    best_val_sim, best_val_loss, patience_counter, best_epoch = -float('inf'), float('inf'), 0, 0
    ...
    try:
        for epoch in range(1, config['epochs'] + 1):
            train_loss, _ = run_epoch(model, dataloaders['train'], text_emb, class_words, label_to_word, 
...
def run_tests(device="cpu"):
    ...
    # 1. Test ImageEncoder
    # -------------------------------
    print("\n[TEST 1] ImageEncoder forward()")

    model = ImageEncoder(proj_dim=64, device=device).to(device)

    fake_images = torch.randn(4, 3, 224, 224).to(device)
    feats, proj = model(fake_images)

    assert feats.shape == (4, 576), f"Expected (4,576), got {feats.shape}"
    assert proj.shape == (4, 64), f"Expected (4,64), got {proj.shape}"

    print("✓ ImageEncoder passed.")

    # -------------------------------
    # 2. Test compute_contrastive_loss
    # -------------------------------
    print("\n[TEST 2] compute_contrastive_loss")

    img_emb = torch.randn(4, 64)
    txt_emb = torch.randn(4, 64)

    loss = compute_contrastive_loss(img_emb, txt_emb, temperature=0.1)
    assert loss > 0, "Loss should be > 0"

    print("✓ Loss function passed.")

    # -------------------------------
    # 3. Test run_epoch (train)
    # -------------------------------
    print("\n[TEST 3] run_epoch() – TRAIN mode")
    ...
    # Fake text embeddings for 3 classes
    text_emb = torch.randn(3, 64).to(device)

    class_words = ["cat", "dog", "car"]
    label_to_word = {0: "cat", 1: "dog", 2: "car"}

    optimizer = torch.optim.AdamW(model.projection.parameters(), lr=1e-3)

    train_loss, train_sim = run_epoch(
        model, train_loader, text_emb, class_words, label_to_word,
        optimizer, temperature=0.1, device=device, mode="train"
    )

    assert isinstance(train_loss, float)
    assert isinstance(train_sim, float)

    print("✓ run_epoch(train) passed.")

    # -------------------------------
    # 4. Test run_epoch (eval)
    # -------------------------------
    print("\n[TEST 4] run_epoch() – EVAL mode")

    val_loss, val_sim = run_epoch(
        model, train_loader, text_emb, class_words, label_to_word,
        optimizer=None, temperature=0.1, device=device, mode="eval"
    )

    assert isinstance(val_loss, float)
    assert isinstance(val_sim, float)
    print("✓ run_epoch(eval) passed.")

    # -------------------------------
    # 5. Test train_with_early_stopping
    # -------------------------------
    print("\n[TEST 5] train_with_early_stopping()")

    # Use tiny dataset & fast config
    dataloaders = {
        "train": train_loader,
        "val": train_loader  # reuse for testing
    }
    ...
    history, best_epoch, best_sim, best_loss = train_with_early_stopping(
        model, dataloaders, text_emb, class_words,
    ...
    # -------------------------------
    # 6. Test CIFAR100Filtered dataset
    # -------------------------------
    print("\n[TEST 6] CIFAR100Filtered dataset")

    dataset = CIFAR100Filtered(split="train")
    ...
    # -------------------------------
    # 7. Test filter_dataset_indices
    # -------------------------------
    print("\n[TEST 7] filter_dataset_indices()")

    dataset = CIFAR100Filtered(split="train")
    valid_labels = {0, 1, 2, 5}
    indices = filter_dataset_indices(dataset, valid_labels)
    
    assert len(indices) > 0, "Should find indices for valid labels"
    assert all(isinstance(i, int) for i in indices), "All indices should be integers"
    
    # Verify all returned indices have valid labels
    for idx in indices[:10]:  # Check first 10
        _, label = dataset[idx]
        assert label in valid_labels, f"Label {label} not in valid_labels"

    # -------------------------------
    # 8. Test create_data_splits
    # -------------------------------
    print("\n[TEST 8] create_data_splits()")

    indices = list(range(100))
    train_idx, val_idx = create_data_splits(indices, val_ratio=0.2, seed=42)
    
    assert len(train_idx) + len(val_idx) == len(indices), "Split should preserve all indices"
    assert len(train_idx) == 80, f"Expected 80 train samples, got {len(train_idx)}"
    assert len(val_idx) == 20, f"Expected 20 val samples, got {len(val_idx)}"
