...
def breadth_first_search(
    ...
    # initialize queue
    # tuple with 4 things: name of the node, the path, node depth, parent node
    ...
    while queue: 
        ...
        # neighbors form the current node
        neighbors = list(G.neighbors(node))
        unvisited_neighbors = [n for n in neighbors if n not in state['visited']]

        print(f" -> Neigbors: {neighbors}")

        print(f" -> Adding the queue: {unvisited_neighbors}")

        print (f" (will explory at {depth + 1})")
...
def depth_first_search(
    ...
    while stack: 
        ...
        # neigbors form the current node
        neighbors = list(G.neighbors(node))
        unvisited_neighbors = [n for n in neighbors if n not in state['visited']]

        # print(f" -> Neigbors: {neighbors}")
        # print(f" -> Adding to stack: {unvisited_neighbors}")
        # print(f" (will explore at depth {depth + 1})")

        # add neighbors to stack
        for neighbor in reversed(unvisited_neighbors):
            stack.append((neighbor, path + [neighbor], depth + 1, node))
...
def uniform_cost_search(
    ...
    while pq:
        ...
        frontier_info = [(n, f"${cost:.1f}") for (cost, n, _, _, _) in sorted(pq)]
        _print_frontier_state(state['step'], frontier_info, 'UCS')

        # pop node 
        current_cost, node, path, depth, parent = heapq.heappop(pq)
        print(f"Expanding: '{node}' with path cost ${current_cost:.1f}")

        if node in state['visited']:
            print(f" -> Skipping '{node}' - already explored with lower cost")
            continue
        ...
        if node == end:
            _print_goal_reached(path, state['max_depth'], max_pq_size, current_cost)
            return path, state['expanded'], state['tree_edges'], state['max_depth'], current_cost
        ...
        print(f" -> Neigbors: {neighbors}")
        print(f" -> Adding to pq: {unvisited_neighbors}")
        ...
        for neighbor in unvisited_neighbors:
            if distance_matrix is not None:
                ...
                if isinstance(distance_matrix, dict):
                    ...
                    edge_cost = distance_matrix[node_to_idx[node], node_to_idx[neighbor]]
            else:
                raise ValueError(f"variable distance_matrix was not provided by the user")

            total_cost = current_cost + edge_cost
            heapq.heappush(pq, (total_cost, neighbor, path + [neighbor], depth + 1, node))
            
            print(f" * '{neighbor}': +${edge_cost} -> total ${total_cost: .1f}")

            if neighbor not in cost_so_far or total_cost < cost_so_far[neighbor]:
                cost_so_far[neighbor] = total_cost
                heapq.heappush(pq, (total_cost, neighbor, path + [neighbor], depth + 1, node))
                print(f" -> Adding '{neighbor}' with new total cost {total_cost:.2f}")
...
class TestSearchAlgorithms(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.weighted_graph = nx.Graph()
        self.weighted_graph.add_edge('A', 'B', weight=1)
        self.weighted_graph.add_edge('B', 'C', weight=5)
        self.weighted_graph.add_edge('A', 'C', weight=2)
    ...
    def test_bfs_reaches_goal(self):
        ...
        with self.suppress_output():
            path, expanded, tree_edges, depth = breadth_first_search(self.graph, 'A', 'E')
    
        self.assertIsInstance(path, list, "BFS path must be a list")
        self.assertGreater(len(path), 0, "BFS returned empty path")
    ...
    def test_dfs_finds_a_path(self):
        ...
        # TODO: Implement the test body
        with self.suppress_output():
            path, expanded, tree_edges, depth = depth_first_search(self.graph, 'A', 'E')
    
        self.assertIsInstance(path, list, "DFS path must be a list")
        ...
        self.assertIn(path, [['A', 'D', 'E'], ['A', 'B', 'C', 'E']], "Unexpected DFS path")
    ...
    def test_ucs_optimal_cost(self):
        ...
        with self.suppress_output():
            result = uniform_cost_search(self.weighted_graph, 'A', 'C', distance_matrix=distance_matrix)
            path, expanded, tree_edges, depth, total_cost = result
    
        self.assertIsNotNone(path, "UCS returned None — path should exist")
        self.assertGreater(len(path), 0)
        self.assertEqual(path[0], 'A')
        self.assertEqual(path[-1], 'C')
    
        for u, v in zip(path, path[1:]):
            self.assertTrue(self.weighted_graph.has_edge(u, v))
    
        expected_path = ['A', 'C']
        expected_cost = 2
    
        self.assertEqual(path, expected_path)
        self.assertAlmostEqual(total_cost, expected_cost, msg="UCS should find optimal cost")
