...
def astar_search(initial: State,
    ...
    add_to_actions = defaultdict(list)
    for a in actions:
        for p in a.add_effects:
            add_to_actions[p].append(a)

    pq = []
    tie = count()

    h0 = heuristic(initial, goal)
    start = SearchNode(f_score=h0, state=initial, action=None, parent=None, g_score=0)
    heapq.heappush(pq, (start.f_score, next(tie), start))
    ...
    while pq and iters < max_iter:
        ...
        if curr.state.satisfies(goal):
            if verbose:
                print(f"A* found plan. Iterations: {iters}, plan length: {curr.g_score}")
            return curr.get_plan()

        if verbose and iters % 5000 == 0:
            print("iters", iters, "open", len(pq), "best_g", len(best_g))

        missing = goal - curr.state.predicates

        candidates = []
        for mg in missing:
            candidates.extend(add_to_actions.get(mg, []))
        ...
        for action in candidates:
            if not curr.state.is_applicable(action):
                continue
            ...
            if new_g < best_g.get(new_state, float("inf")):
                ...
                node = SearchNode(f_score=new_f, state=new_state, action=action, parent=curr, g_score=new_g)
                heapq.heappush(pq, (new_f, next(tie), node))

    if verbose:
        print(f"A* no solution")
    return None
